...
var APIURL = (PhoneGap && PhoneGap.env && PhoneGap.env.app ? 'https://myvideotraining.azurewebsites.net/' : '/') + 'api/';
...
angular.module('app').controller('mainCtrl', function ($scope, $http) {
    ...
    $scope.loadSessions = function () {
        if (typeof (Storage) != "undefined") {
            var ss = localStorage.getItem("sessions");
            if (ss) {
                $scope.sessions = ss;
            }
        }
    ...
    $scope.onChooseAssignment = function (assignment) {
        ...
        $scope.vid.src = assignment.mediaUrl;
        $scope.vid.load();
    };
	...
	$scope.healthTap = function (e) {
	    ...
	    $scope.healthIndicator = pointer.clientX - indicator.width / 2;
	    $scope.$apply();
	};
